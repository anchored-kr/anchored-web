      ...
      for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
          ...
          const dist = Math.sqrt(
            Math.pow(x - canvas.width / 2, 2) + Math.pow(y - canvas.height * 0.4, 2)
          );
          const wave = Math.sin(dist * 0.003 - time * 2) * 0.5 + 0.5;
          ctx.beginPath();
          ctx.arc(x, y, 1.2, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(255, 255, 255, ${wave * 0.1})`;
          ctx.fill();
        }
      }
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        <motion.p
          variants={item}
          className="mt-10 text-lg md:text-xl text-dim max-w-2xl mx-auto leading-relaxed"
        >
          Anchored is building a LiveOps engine
          <br className="hidden sm:block" />
          for creator-led games.
        </motion.p>
